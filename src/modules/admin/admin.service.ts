...
export class AdminService {
  ...
  async findAll(): Promise<{ admins: Admin[], count: number }> {
    const [admins, count] = await this.adminRepository.findAndCount({ where: { state: true } });
    return { admins, count };
  }  
  ...
  async update(id: number, updateData: Partial<Admin>, requestTokenId: number): Promise<Admin> {

    if (id !== requestTokenId) {
      throw new UnauthorizedException('You do not have permission to update this admin.');
    }
  ...
  async findOneBy(criteria: { id: number; state: boolean }): Promise<Admin | undefined> {
